    ...
    <CardShell icon={PackageSearchIcon} title={`Order #${order.id}`}>
      <div className="mb-3 flex items-center justify-between gap-2">
        <div>
          ...
          <p className="truncate text-xs text-muted-foreground">{order.shippingAddress}</p>
        </div>
        <StatusBadge status={order.status} kind="order" />
      </div>
      <ul className="-mx-1 divide-y">
        {items.map((item) => (
          <li key={item.bookId} className="flex items-center justify-between gap-2 px-1 py-2">
