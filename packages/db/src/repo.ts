...
function getStockForBook(id: number): number {
  const row = db.select({ stock: books.stock }).from(books).where(eq(books.id, id)).get();
  return row?.stock ?? 0;
}
...
export function searchBooks(opts: {
  ...
  if (opts.query && opts.query.trim().length > 0) {
    ...
    conds.push(or(like(books.title, q), like(books.author, q), like(books.description, q))!);
  }
...
export function getRecommendations(userId: string, limit = 5): Book[] {
  ...
  if (purchasedIds.length > 0) {
    const purchasedBooks = db.select().from(books).where(inArray(books.id, purchasedIds)).all();
  ...
  const candidates = db
    .select()
    .from(books)
    .where(purchasedIds.length > 0 ? not(inArray(books.id, purchasedIds)) : undefined)
    .all()
    .map((book) => ({
      book,
...
export function addToCart(userId: string, bookId: number, quantity = 1): Cart {
  ...
  if (existing) {
    db.update(cartItems).set({ quantity: newQty }).where(eq(cartItems.id, existing.id)).run();
  } else {
...
export function updateCartItemQty(userId: string, bookId: number, quantity: number): Cart {
...
export function getAccountSummary(userId: string): AccountSummary {
  ...
  const orderStats = db
    ...
    .from(orders)
    .where(
      and(eq(orders.userId, userId), inArray(orders.status, ["paid", "shipped"] as OrderStatus[])),
    )
    .get();
  ...
  const openTickets = db
    ...
    .from(supportTickets)
    .where(
      and(eq(supportTickets.userId, userId), eq(supportTickets.status, "open" as TicketStatus)),
    )
    .get();
...
export function getOrderDetail(orderId: number, userId?: string): OrderDetail | undefined {
...
export function createOrder(userId: string, shippingOptionId: string): OrderDetail {
  const cart = getCart(userId);
...
export function searchFaq(opts: { query?: string; category?: string }): Faq[] {
  const conds = [];
  if (opts.query && opts.query.trim().length > 0) {
    const q = `%${opts.query.trim()}%`;
...
export function createSupportTicket(userId: string, subject: string, body: string): SupportTicket {
  const result = db.insert(supportTickets).values({ userId, subject, body, status: "open" }).run();
...
export function getSupportTicket(ticketId: number, userId?: string): SupportTicket | undefined {
...
export function createBook(input: Omit<Book, "createdAt">): Book {
  const { id, title, author, genre, priceCents, description, rating, stock, coverUrl, isbn } =
    input;
  db.insert(books)
...
export function updateTicketStatus(id: number, status: TicketStatus): SupportTicket | undefined {
  const existing = db.select().from(supportTickets).where(eq(supportTickets.id, id)).get();
  if (!existing) return undefined;
  db.update(supportTickets).set({ status }).where(eq(supportTickets.id, id)).run();
